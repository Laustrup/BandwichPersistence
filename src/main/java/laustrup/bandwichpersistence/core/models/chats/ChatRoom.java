...
public class ChatRoom extends Model<ChatRoom.Id, Signature.UUID> {
   ...
   */
  private final Seszt<Message> _messages;
  ...
  public ChatRoom(ChatRoom.DTO chatRoom) {
    this(
        ...
        chatRoom.getTitle(),
        Seszt.copy(chatRoom.getMails(), Message::new),
        Seszt.copy(chatRoom.getChatters(), UserService::from),
        chatRoom.getTimestamp()
    );
  }
  ...
  public ChatRoom(
    ...
    super(id, timestamp);
    _title = determineChatRoomTitle(title);
    _chatters = chatters;
    _messages = messages;
  }
  ...
  public ChatRoom(String title, Seszt<Message> messages, Seszt<User<? extends User.Id>> chatters) {
    this(null, title, messages, chatters, null);
  }
   ...
   */
  public Seszt<Message> add(Message message) {
  ...
  @Getter
  @FieldNameConstants
  @Table("chat_room_templates")
  public static class Template extends Model<Template.Id, Signature.UUID> {

    private final String _title;

    private final Seszt<BusinessUser<? extends User.Id>> _chatters;
    ...
    public static class DTO extends ModelDTO<ChatRoom.Template.Id, Signature.UUID, java.util.UUID> {
      ...
      private final Set<BusinessUserDTO<? extends User.Id>> chatters;
  ...
  public static class DTO extends ModelDTO<ChatRoom.Id, Signature.UUID, java.util.UUID> {
     ...
     * All the Mails that has been sent will be stored here.
     */
    private final Set<Message.DTO> mails;

    /**
     * The Users, except the responsible, that can write with each other.
     */
    private final Set<UserDTO<? extends User.Id>> chatters;
